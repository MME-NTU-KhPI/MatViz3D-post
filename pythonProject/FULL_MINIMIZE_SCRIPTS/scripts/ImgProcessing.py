...
def main():
    path = r'D:\University\MatViz\1FULL_MINIMIZE_SCRIPTS\Target_img\WE43-0P'
...
def process(filename, plot_orientation=False, plot_hist=False):
    ...
    # Створюємо ОДНЕ вікно для фінального результату
    ...
    for props in regions:
        is_on_edge = (np.any(props.coords[:, 0] == 0) or
                      np.any(props.coords[:, 0] == h - 1) or
                      np.any(props.coords[:, 1] == 0) or
                      np.any(props.coords[:, 1] == w - 1))
        ...
        # НАКЛАДАННЯ ТЕКСТУ (Виконується прямо на фоні 'colored')
        if ENABLE_GRAIN_LABELS:
            if is_on_edge:
                ax.text(x0, y0, "ø", color='black', fontsize=FONT_SIZE,
                        ha='center', va='center', weight='bold',
                        bbox=dict(facecolor='white', alpha=0.6, pad=0.1, edgecolor='none'))
            else:
                label_text = f"A: {props.area:.0f}"
                it = props.inertia_tensor
                label_text += f"\nIxx/A: {it[0, 0] / props.area:.4f}"
                label_text += f"\nIxy/A: {it[0, 1] / props.area:.4f}"
                label_text += f"\nIyy/A: {it[1, 1] / props.area:.4f}"

                ax.text(x0, y0, label_text, color='black', fontsize=FONT_SIZE,
                        ha='center', va='center',
                        bbox=dict(facecolor='white', alpha=0.7, pad=0.1, edgecolor='none'))

        # Фільтрація для розрахунків (крайові зерна не йдуть в CSV)
        if is_on_edge:
            continue
    ...
    ax.imshow(bwimage, alpha=0.3, cmap='gray')
