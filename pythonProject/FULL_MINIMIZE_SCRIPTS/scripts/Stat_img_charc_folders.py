...
def apply_log_transform(df):
    """
    Тепер використовує Arcsinh замість Log для підтримки
    від'ємних значень компонент тензора інерції.
    """
...
def process_file(folder_path, csv_file):
    """
    Process single CSV file: read, clean, stats (normal & log), save, plot.
    """
    ...
    # Генеруємо звичайну статистику
    stats_df = generate_statistics_df(df_processed)

    # 2. Логарифмічна обробка
    # (Тут створюється DataFrame з логарифмованими значеннями)
    df_log = apply_log_transform(df_processed.copy())

    # Генеруємо статистику для логарифмічного простору
    stats_log_df = generate_statistics_df(df_log)

    # --- Збереження результатів ---
    output_dir = os.path.join(folder_path, 'processed_output')
    os.makedirs(output_dir, exist_ok=True)

    # 1. Зберігаємо очищені дані (оригінальні значення)
    df_processed.to_csv(os.path.join(output_dir, f"processed_{csv_file}"), index=False)

    # 2. === НОВЕ: Зберігаємо дані в логарифмічному просторі ===
    # Всі колонки тут вже переведені в np.log (функцією apply_log_transform)
    df_log.to_csv(os.path.join(output_dir, f"processed_Arcsinh_{csv_file}"), index=False)

    # 3. Файл статистики (оригінальні дані)
    stats_df.to_csv(os.path.join(output_dir, f"statistics_{csv_file}"), index=False)

    # 4. Файл статистики (логарифмічні дані)
    stats_log_df.to_csv(os.path.join(output_dir, f"Arcsinh_statistics_{csv_file}"), index=False)

    # Графіки будуємо за звичайними даними (як і було)
    plot_histograms(df_raw, df_processed, output_dir)

    print(f"File '{csv_file}' processed. Saved normal data, Arcsinh data, and stats to: {output_dir}")


# ===== Main =====
folders_to_process = [
    # r"D:\\Універ\\ДИПЛОМ\\TEST_alg\\optimize_models_update\\AZ31_iA"
    # r"D:\University\MatViz\1FULL_MINIMIZE_SCRIPTS\Target_img\AZ31_iB"
    r"D:\University\MatViz\1FULL_MINIMIZE_SCRIPTS\Target_img\WE43-0P"
]
