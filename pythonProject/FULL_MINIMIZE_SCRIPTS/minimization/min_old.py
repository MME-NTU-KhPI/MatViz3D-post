...
# TODO:  Метод оптимізації, ('SLSQP', 'L-BFGS-B', 'Dual Annealing', 'basinhopping', 'Differential Evolution')
...
exe_path = r"D:\Project(MatViz3D)\Random\script_minimization\debug\MatViz3D.exe"
launcher = MatViz3DLauncher(exe_path)

# TODO:  Створення каталогу для виведення
output_folder = r"D:\Project(MatViz3D)\Random\Paper_Optimisation\test\inertia_tensor\5_SMAPE_Dual-Annealing_AZ31_ia_I_Anisotropy"
...
csv_params_path = os.path.join(output_folder, "run_parameters.csv")
csv_headers = [
...
def log_run_parameters(params):
    file_exists = os.path.isfile(csv_params_path)
    with open(csv_params_path, mode='a', newline='', encoding='utf-8') as csvfile:
...
def start(size, concentration, halfaxis_a, halfaxis_b, halfaxis_c, orientation_angle_a, orientation_angle_b,
    ...
    print(f"Запуск MatViz3D з параметрами: [{size}, {concentration}, {halfaxis_a}, {halfaxis_b}, {halfaxis_c}, "
          f"{orientation_angle_a}, {orientation_angle_b}, {orientation_angle_c}, {wave_coefficient}]")
...
def minimize_properties(params):
    ...
    # TODO: шлях до файлів
    output_file = "cube_output.csv"
    output_props_file = "properties_output.csv"
...
def optimize_properties():
    # TODO: Обмеження
    # bounds = [(0, 1), (0.1, 90), (1, 1.5), (1, 1.5), (1, 1.5), (0, 360), (0, 180), (0, 360)]
    bounds = [(0, 1), (0.01, 0.2), (1, 1.5), (1, 1.5), (1, 1.5), (0, 180), (0, 90), (0, 180)]
    bounds = [(0, 1), (0.01, 0.2), (0.51, 1.5), (0.51, 1.5), (0.51, 1.5), (0, 180), (0, 90), (0, 180)] #Кути 0
